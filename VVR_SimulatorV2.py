...
class VVR_Simulator():
    ...
    def get_out_tensor(self, gpu=False, cc=False):
        in_tensor = torch.zeros(self.num_model)
        place_holder=torch.zeros(np.abs(self.num_color-self.num_model))
        last_tensor = torch.zeros(self.num_color)
        m_hist = torch.Tensor(self.bank.front_hist()).float()
        m_hist2 = torch.Tensor(self.bank.front_hist(scope=2)).float()
        m_hist3 = torch.Tensor(self.bank.front_hist(scope=3)).float()
        m_hist = torch.cat([m_hist, place_holder])
        m_hist2 = torch.cat([m_hist2, place_holder])
        m_hist3 = torch.cat([m_hist3, place_holder])
