...
for set in range(6):
    ...
    policy_net = Q_net_VVR(c=NUM_COLORS, m=NUM_MODELS, num_lane=NUM_LANES, lane_length=LANE_LENGTH, ccm=sim.ccm, include_last=True).cuda()
    target_net = Q_net_VVR(c=NUM_COLORS, m=NUM_MODELS, num_lane=NUM_LANES, lane_length=LANE_LENGTH, ccm=sim.ccm, include_last=True).cuda()
